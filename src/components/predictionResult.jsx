    ...
    <div className="mt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        <div className="flex justify-center items-center bg-card rounded-lg p-6 flex flex-col gap-4">
          ...
          <div
            className={`p-7 text-6xl font-bold ${
              increased ? "text-blue-500" : "text-red-600"
            }`}
          >
            ${avg_predictions.toFixed(2)}
          </div>
          <div className="text-l text-muted-foreground text-center">
            Compared to the previous 10 days, the next 10 days of the stock are
            expected to {increased ? "increase" : "decrease"} by{" "}
            <span className={`${increased ? "text-blue-500" : "text-red-600"}`}>
              {((avg_predictions / avg_last10 - 1) * 100).toFixed(2)}%
            </span>{" "}
            {increased ? ":D" : ":'("}
          </div>
        </div>
